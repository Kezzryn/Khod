        ...
        IComparisonOperators<Point2D, Point2D, bool>
{
    ...
    public readonly IEnumerable<Point2D> GetAllNeighbors(int radius = 1)
    {
        int lBound = -radius;
        int range = (radius * 2) + 1;

        foreach ((int x, int y) in from y in Enumerable.Range(lBound, range)
                                   from x in Enumerable.Range(lBound, range)
                                   where (!(x == 0 && y == 0))
                                   select (x, y))
        {
    ...
    public readonly IEnumerable<Point2D> GetNeighborsAtRadius(int radius)
    {
        int lBound = -radius;
        int range = (radius * 2) + 1;
        ...
        foreach ((int x, int y) in from y in Enumerable.Range(lBound, range)
                                   from x in Enumerable.Range(lBound, range)
                                   where (x == radius || x == lBound || y == radius || y == lBound)
                                   select (x, y))
        {
            yield return new Point2D(x + X, y + Y);
        }
    ...
    public readonly bool IsOnGridLine(Point2D other)
    {
        return RelativeQuadrent(other) == Quadrant.OnGridLine;
    }
